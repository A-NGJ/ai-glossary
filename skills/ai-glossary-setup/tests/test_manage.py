...
class CurateFromMessagesSkipMissingTest(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.data_home = self.root / "config" / "ai-glossary"
        self.claude = self.root / "claude" / "CLAUDE.md"
        self.claude.parent.mkdir(parents=True)
        self.claude.write_text("existing\n", encoding="utf-8")
        self.agents = self.root / "codex" / "AGENTS.md"

    def tearDown(self):
        self.temp.cleanup()

    def test_nonexistent_target_is_skipped_by_default(self):
        self.assertFalse(self.agents.exists())

        manage.curate_from_messages(
            self.data_home, self.claude, self.agents, messages=[]
        )

        self.assertFalse(self.agents.exists())
        self.assertIn(manage.START, self.claude.read_text(encoding="utf-8"))

    def test_nonexistent_target_is_written_when_skip_missing_is_false(self):
        self.assertFalse(self.agents.exists())
        ...
        self.assertIn(manage.START, self.agents.read_text(encoding="utf-8"))
