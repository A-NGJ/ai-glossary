...
When setup changes a file it prints a restart reminder: start a new agent
session so the updated glossary block is in context. A run with nothing to
change prints only `setup already complete`.
